...
void lcd_rotatingBanner(char dispText[])
{
    ...
    char temp1[21] = {' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','\0'}; // 20 characters of spaces

    while (1) //infinite loop
    {
        for (i = 0; i <= strlen(dispText) + 19; i++)
        {
            if(i > strlen(dispText)) //this conditional prints the spaces
            {
                for (j = 1; j <= 19; j++) //shift through all spaces
                {
                    temp1[j - 1] = temp1[j];
                }
            ...
            else //this conditional shifts the letters
            {
                for (j = 1; j <= 19; j++) //shift through all spaces
                {
                    temp1[j - 1] = temp1[j];
